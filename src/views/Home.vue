...
const log = createLogger('retire:home');
...
const getAnnualIncome = () => {
  ...
  if (nameMatch) {
    log.log('name match:', nameMatch[1]);
    return parseInt(nameMatch[1]) * 12;
  }

  // 如果都无法解析，假设余额是月收入
  log.log('fallback, balance*12:', salaryAccount.data.balance * 12);
