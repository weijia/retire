...
#### 10.2.4 社平工资数据集（avg_wage_dataset）

```typescript
// 单年社平工资数据
interface AvgWageYearData {
  year: number;              // 年份
...
// 社平工资数据集（按城市/地区）
interface AvgWageDataSet extends BaseDocument {
...
**文档ID**: `avg_wage_main`（单例，每个用户只有一份）

**索引字段**: `data.city`, `data.province`
```
...
- **月缴费基数**：按月数加权平均
- **社平工资**：优先使用导入的社平工资数据，未导入时暂按缴费基数估算（100%）
- **缴费比例**：默认个人 8%、单位 16%
...
支持导入本地历年社会平均工资数据，用于精确计算养老金缴费指数。
...
- `city`: 城市名称（必填）
- `province`: 省份名称（必填）
- `scope`: 统计范围，`city` | `province` | `national`（必填）
- `description`: 数据说明（可选）
- `years`: 历年数据数组（必填）
  - `year`: 年份，整数（必填）
...
用户可复制以下提示词，发送给 ChatGPT/Claude 等 AI 生成 JSON 数据：

```
请帮我生成一份历年社会平均工资数据，格式如下：
...
- **文件位置**：`src/utils/avgWageImport.ts`
- **存储类型**：`avg_wage_dataset`
- **Store**：`src/stores/avgWage.ts`
- **页面**：`src/views/AvgWage.vue`
- **路由**：`/avg-wage`
...
| `src/utils/avgWageImport.ts` | 社平工资 JSON 导入工具（含 AI 提示词生成） |
| `src/stores/avgWage.ts` | 社平工资数据 Store |
...
| `src/views/Pension.vue` | 养老金测算页（含缴费阶段管理、浙里办导入、社平工资入口） |
...
| `src/views/AvgWage.vue` | 社平工资管理页（含 AI 提示词、JSON 导入） |
...
- `GiteeSyncConfig` 接口
- `AvgWageYearData` 接口（单年社平工资数据）
- `AvgWageDataSet` 接口（社平工资数据集）
